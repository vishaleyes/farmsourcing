...
function ngLanguage($locale="eng") {
...
if(!isset($this->_languageTable[$locale]))
{
...
$this->_unloadTranslationTable($locale, $path);
	...
	foreach ($this->_loadedTranslationTables[$this->_currentLanguage] as $table) {
		if (isset($trans[$table][$key])) {
			$result=str_replace('_SITENAME_NO_CAPS_',_SITENAME_NO_CAPS_,$trans[$table][$key]);
			$result=str_replace('_SITENAME_CAPS_',_SITENAME_CAPS_,$result);
			$result=str_replace('_SITENAME_',_SITENAME_,$result);
			return $result;
		}
	}
	}
